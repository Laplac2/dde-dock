...
void MultiScreenWorker::onRequestNotifyWindowManager()
{
    ...
    qDebug() << "screen with:" << m_screenRawWidth << ", screen height:" << m_screenRawHeight;
...
const QPoint MultiScreenWorker::rawXPosition(const QPoint &scaledPos)
{
    ...
    return screen ? screen->geometry().topLeft() + (scaledPos - screen->geometry().topLeft()) * screen->devicePixelRatio()
                  : scaledPos;
}
